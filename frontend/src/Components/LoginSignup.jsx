...
const LoginSignup = () => {
  ...
  const handleSendOtp = async (email) => {
    try {
      await axios.post("http://127.0.0.1:9000/api/users/send-otp", { email });
      setOtpSent(true);
  ...
  const handleVerifyOtp = async () => {
    try {
      await axios.post("http://127.0.0.1:9000/api/users/verify-otp", {
        email: emailToVerify,
        otp: otp,
      });
      setOtpVerified(true);
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      if (isLogin) {
        const res = await axios.post("http://127.0.0.1:9000/api/users/login", {
          email: values.email,
